...
app.get('/login', (req, res) => {
  res.send(`
    ...
    <body>
      <div class="login-box">
        <h1>ÃƒÂ°Ã…Â¸Ã‚Â¦Ã¢â€šÂ¬ Swissclaw Hub</h1>
        <form id="loginForm">
...
async function initDb() {
  ...
  try {
    ...
    await pool.query(`
      ...
      VALUES
        ('backlog', 'Backlog', 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â', '#6b7280', 0),
        ('todo', 'To Do', 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹', '#3b82f6', 1),
        ('inProgress', 'In Progress', 'ÃƒÂ°Ã…Â¸Ã…Â¡Ã¢â€šÂ¬', '#f59e0b', 2),
        ('review', 'Review', 'ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â€šÂ¬', '#8b5cf6', 3),
        ('done', 'Done', 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦', '#10b981', 4),
        ('waiting', 'Waiting', 'ÃƒÂ¢Ã‚ÂÃ‚Â¸ÃƒÂ¯Ã‚Â¸Ã‚Â', '#ef4444', 5)
...
// Tasks API - operator's action items from kanban.md
function parseTasksFromKanban() {
  try {
    ...
    const possiblePaths = [
      ...
      path.join(process.cwd(), '..', 'kanban', 'kanban.md'),
      path.join('/home/operator/.openclaw/workspace', 'kanban', 'kanban.md'),
    ...
    content.split('\n').forEach(line => {
      if (line.includes('## ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â¤ operator\'s Action Items')) {
        inActionItems = true;
      } else if (line.startsWith('## ')) {
        inActionItems = false;
      } else if (inActionItems && line.trim().startsWith('- [ ]')) {
        // Parse checkbox items: - [ ] **Title** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â description
        const match = line.match(/^- \[ \] \*\*(.+?)\*\*\s*[-ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â]\s*(.+)$/);
        if (match) {
          tasks.push({
            id: id++,
            title: match[1].trim(),
