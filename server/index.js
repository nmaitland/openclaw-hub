...
function parseKanbanMarkdown() {
  try {
    ...
    content.split('\n').forEach(line => {
      if (line.includes('## Ã°Å¸â€œâ€¹ To Do')) {
        currentSection = 'todo';
      } else if (line.includes('## Ã°Å¸Å¡â‚¬ In Progress')) {
        currentSection = 'inProgress';
      } else if (line.includes('## Ã¢Å“â€¦ Done')) {
        currentSection = 'done';
      } else if (line.startsWith('## ')) {
        currentSection = null;
      } else if (currentSection && line.trim().startsWith('- **')) {
        // Parse format: - **Title** Ã¢â‚¬â€ description
        const match = line.match(/^- \*\*(.+?)\*\*\s*[-Ã¢â‚¬â€]\s*(.+)$/);
        if (match) {
...
function parseTasksFromKanban() {
  try {
    ...
    content.split('\n').forEach(line => {
      if (line.includes('## Ã°Å¸â€˜Â¤ operator\'s Action Items')) {
        inActionItems = true;
      } else if (line.startsWith('## ')) {
        inActionItems = false;
      } else if (inActionItems && line.trim().startsWith('- [ ]')) {
        // Parse checkbox items: - [ ] **Title** Ã¢â‚¬â€ description
        const match = line.match(/^- \[ \] \*\*(.+?)\*\*\s*[-Ã¢â‚¬â€]\s*(.+)$/);
        if (match) {
          tasks.push({
            id: id++,
